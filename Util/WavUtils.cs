...
/// <summary>
/// This is changed from the original, to work better with all potential types of wav files,
/// since the original was simple and assumed a lot of formatting according to the riff wav specification.
///
        ...
        public static AudioClip ToAudioClip(byte[] fileBytes, string name = "wav")
        {
            ...
            while (currentHeaderOffset < fileBytes.Length)
            {
                ...
                if (fmtChunkID == SubchunkID)
                {
                    ...
                    if (FormatCode(headerData.AudioFormat) != "PCM")
                    {
                        Debug.LogError($"ERROR: Format doesn't match! Only \"PCM\" is supported.");
                        return null;
                    }
            ...
            switch (headerData.BitsPerSample)
            {
                ...
                case 32:
                    ...
                    break;
                default:
                    Debug.LogError($"ERROR: Format doesn't match! {headerData.BitsPerSample} bit depth is not supported.");
                    return null;
            }
